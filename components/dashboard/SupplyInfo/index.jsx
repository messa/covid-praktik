...
export default function SupplyInfo({ supplyUpdates }) {
  ...
  const currentSupplies = supplies.reduce(
    (res, supply) => {
      return {
        ffp3:
          res.ffp3 -
          parseInt(supply.ffp3Consumed) +
          parseInt(supply.ffp3Received),
        ffp2:
          res.ffp2 -
          parseInt(supply.ffp2Consumed) +
          parseInt(supply.ffp2Received),
        shield:
          res.shield -
          parseInt(supply.shieldConsumed) +
          parseInt(supply.shieldReceived),
        suit:
          res.suit -
          parseInt(supply.suitConsumed) +
          parseInt(supply.suitReceived),
      }
    },
    {
      ffp3: 0,
    ...
    <div>
      <Heading text={'Vybavení ordinace'}>
        <Button
          ...
          small
          onClick={() => modalController[1](visible => !visible)}
        >
...
function SupplyConsumed({ amount }) {
  return (
    <span title={'Spotřebované'}>
      <ArrowDown className={classNames(styles.icon, styles.down)} />
      &nbsp;{amount}
    </span>
  )
}
function SupplyRecieved({ amount, fromState }) {
  return (
    <span title={'Nové'}>
      <ArrowUp className={classNames(styles.icon, styles.up)} />
      &nbsp;{amount} <span title={'Od státu'}>({fromState})</span>
    </span>
  )
}
