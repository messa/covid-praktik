...
export async function findUsersByOfficeId(officeId) {
  return await User.find({ officeId }).exec()
}
...
export async function getUserByEmailAndPassword({ emailAddress, password }) {
  ...
  if (!user) {
    console.info(
      `User with emailAddress ${JSON.stringify(emailAddress)} not found`
    )
    return null
  }
  ...
  if (passwordMatches === true) {
    ...
    console.info(
      `Password mismatch for user with emailAddress ${JSON.stringify(
        emailAddress
      )}`
    )
    return null
